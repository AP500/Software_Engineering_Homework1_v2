...
def request_leave():
    ...
    if request_made:
        flash(
            f"You have already submitted a leave request for this date. Existing request: {request_made}"
        )
        return redirect(url_for("index"))
    ...
    new_leave_request = LeaveRequest(
        username=current_user.username, leave_date=leave_date, reason=reason
    )
...
def already_requested(username, leave_date):
    existing_leave_request = LeaveRequest.query.filter_by(
        username=username, leave_date=leave_date
    ).first()
